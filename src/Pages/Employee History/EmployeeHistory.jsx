...
const EmployeeHistory = () => {
  ...
  const fetchEmployees = async () => {
    ...
    try {
      const response = await fetch(`https://employee-attendance-dr6b.onrender.com/admin/dashboard?page=${currentPage}&limit=10`, {
        method: 'GET',
        headers: {
                ...
                <tbody>
                  {displayedEmployees.map((employee, index) => (
                    <tr key={employee.id}>
                      ...
                      <td>
                        <button onClick={() => viewEmployeeProfile(employee._id)}>
                          <FaMapMarkerAlt className="history-icon" /> History
                        </button>
                      </td>
                    </tr>
                  ))}
